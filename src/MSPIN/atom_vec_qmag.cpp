...
AtomVecQmag::AtomVecQmag(LAMMPS *lmp) : AtomVec(lmp)
{
  molecular = 1;
  ...
  atom->molecule_flag = atom->q_flag = atom->qm_flag = 1;
}
...
void AtomVecQmag::grow(int n)
{
  ...
  qm = memory->grow(atom->qm,nmax,"atom:qm");
...
void AtomVecQmag::grow_reset()
{
  tag = atom->tag; type = atom->type;
  mask = atom->mask; image = atom->image;
  x = atom->x; v = atom->v; f = atom->f;
  q = atom->q; qm = atom->qm; molecule = atom->molecule;
...
void AtomVecQmag::copy(int i, int j, int delflag)
{
  int k;
...
int AtomVecQmag::pack_comm(int n, int *list, double *buf,
...
int AtomVecQmag::pack_comm_vel(int n, int *list, double *buf,
                               int pbc_flag, int *pbc)
{
  int i,j,m;
...
void AtomVecQmag::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecQmag::unpack_comm_vel(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecQmag::pack_reverse(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecQmag::unpack_reverse(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecQmag::pack_border(int n, int *list, double *buf,
                             int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz;
...
int AtomVecQmag::pack_border_vel(int n, int *list, double *buf,
                                 int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecQmag::pack_border_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
void AtomVecQmag::unpack_border(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecQmag::unpack_border_vel(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecQmag::unpack_border_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecQmag::pack_exchange(int i, double *buf)
{
  int k;
...
int AtomVecQmag::unpack_exchange(double *buf)
{
  int k;
...
int AtomVecQmag::pack_restart(int i, double *buf)
{
  int k;
...
int AtomVecQmag::unpack_restart(double *buf)
{
  int k;
...
void AtomVecQmag::create_atom(int itype, double *coord)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecQmag::data_atom(double *coord, imageint imagetmp, char **values)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
  ...
  // printf("-- Atom %d : %d %d %d %lf %lf %lf %lf %lf\n", nlocal,
  // 		tag[nlocal], molecule[nlocal], type[nlocal],
  // 		q[nlocal], x[nlocal][0], x[nlocal][1], x[nlocal][2], qm[nlocal]);
}
...
int AtomVecQmag::data_atom_hybrid(int nlocal, char **values)
{
  molecule[nlocal] = ATOTAGINT(values[0]);
  q[nlocal] = atof(values[1]);
  qm[nlocal] = atof(values[2]);
...
void AtomVecQmag::pack_data(double **buf)
{
  int nlocal = atom->nlocal;
...
int AtomVecQmag::pack_data_hybrid(int i, double *buf)
{
...
void AtomVecQmag::write_data(FILE *fp, int n, double **buf)
{
  for (int i = 0; i < n; i++)
    fprintf(fp,TAGINT_FORMAT " " TAGINT_FORMAT
            " %d %-1.16e %-1.16e %-1.16e %-1.16e %d %d %d %-1.16e\n",
...
int AtomVecQmag::write_data_hybrid(FILE *fp, double *buf)
{
  fprintf(fp," " TAGINT_FORMAT " %-1.16e  %-1.16e",(tagint) ubuf(buf[0]).i,
  							buf[1], buf[2]);
  return 3;
}
...
bigint AtomVecQmag::memory_usage()
{
  bigint bytes = 0;
  ...
  if (atom->memcheck("qm")) bytes += memory->usage(qm,nmax);
